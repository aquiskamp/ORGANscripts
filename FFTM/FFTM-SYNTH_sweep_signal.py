__author__ = 'Aaron'
''''Inject signal to calibrate amp'''
import pyvisa
import time
import datetime
import numpy as np
...
exp_name = 'LNA_15_29_cal_11_40_mixer_injection'
filepath = p.home()/'Desktop'/'Aaron'/'Experiments'/'thermal_noise'/exp_name

mode_f = np.arange(15e9,20.25e9,0.1e9)
psd = False   #PSD?
...
signal.write(":SOUR:POW:LEV:IMM:AMPL " + str(signal_power)+" dBm")
signal.write(":OUTP:STAT ON")
signal.write(":OUTP:MOD:STAT OFF")
...
for idx, mode in enumerate(tqdm(mode_f)):
    ...
    with h5py.File(filepath / p(exp_name + '.hdf5'), 'a') as f:
        ...
        mag.attrs['signal_freq'] = mode
        mag.attrs['signal_power'] = signal_power
        mag.attrs['sg_f'] = sg_f
        try:
            f['Freq']
        except:
