...
filepath = r'C:\Users\equslab\Desktop\ORGAN_15GHz\roomtemp_motor_test'
...
#time.sleep(3600*14) #sleep for 12hrs while cooling down 
# Starting Scan:
...
for ato_pos in ato_pos_vals:
    print("Setting position to ato_pos = " + str(ato_pos) + " " + "Steps")
    ...
    if ato_pos == 0 or idx == 0:  # since send a 0 instructs the stage to move continuously
        ...
        inst.write("step" + up_down + " " + "1" + " " + str(ato_step)) #steps the difference between the current pos and previous pos
        time.sleep(1)  # need to sleep otherwise grounds instantly
        inst.write('setm 1 gnd')
    time.sleep(0.1)  # Just wait to stabilize# Attocube code ends
