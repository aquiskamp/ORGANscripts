...
move_to_start = True #use encoder to move to starting pos (deg)
start_pos = 196
...
if move_to_start:
    ...
    anc.setTargetPosition(ax['y'], start_pos)  # set axis, set degrees
    anc.setTargetRange(ax['y'], 1e-3)  # set precision
    print("Moving to ato_pos: %s degrees" % (start_pos))
...
anc.setAxisOutput(ax['y'], 1, 1) #set output to axis, (1) enable (0) disable, autodisable when stop (1)
anc.setTargetPosition(ax['y'],ato_step) # set axis, set degrees
anc.setTargetRange(ax['y'],1e-3) # set precision
target = 0
for idx,ato_pos in enumerate(tqdm(ato_pos_vals)):
    ...
    if ato_pos == 0 or idx == 0: #since send a 0 instructs the stage to move continuously
        anc.setAxisOutput(ax['y'], 0, 1)  # set output to axis, (1) enable (0) disable, autodisable when stop (1)
    else:
        anc.startSingleStep(ax['y'],) #axisN Axis number (0 ... 2)backward	If the step direction is forward (0) or backward (1)
        time.sleep(ato_step/setFreq['x']+0.5)

    current_pos = anc.getPosition(ax['y'])
    if idx == 0:
        print("Starting at ato_pos: %s degrees"%(current_pos))
        # do nothing
    else:
        print("Moving to ato_pos: %s degrees" % (current_pos + ato_pos))
        anc.startAutoMove(ax['y'], 1, 1)  # axis, enable/disable, absolute (0) or relative to the current position (1)
        while target == 0:
            connected, enabled, moving, target, eotFwd, eotBwd, error = anc.getAxisStatus(ax['y'])  # find bitmask of status
            if target == 0:
                None
